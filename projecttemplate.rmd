...
## An overview of the data
  This red wine dataset consists of 1599 observations and 12 variables, 11 of which are chemicals. The       variables are fixed.acidity, volatile.acidity, citric.acid, residual.sugar, chlorides,                     free.sulfur.dioxide, total.sulfur.dioxide, density, pH, sulphates, alcohol and quality.
...
df$quality.level<-factor(df$quality.level,levels = c("low","middel","high"),ordered = TRUE)
```

# Univariate Plots Section

```{r echo=FALSE, Univariate_Plots}
qplot(factor(quality), data=df, geom="bar", xlab="Quality") + 
  theme_bw()
summary(df$quality)
```
...
```{r echo=FALSE}
qplot(fixed.acidity,data = subset(df,!is.na(fixed.acidity)), xlab="Fixed acidity ",binwidth=0.1) + theme_bw()
```

The distribution of fixed acidity is right skewed, and concentrated between 7 and 9.

```{r echo=FALSE}
qplot(volatile.acidity,data = subset(df,!is.na(volatile.acidity)), xlab="Volatile acidity",binwidth=0.01) + theme_bw()
...
```{r echo=FALSE}
qplot(citric.acid,data = subset(df,!is.na(citric.acid)), xlab="Citric acid",binwidth=0.01) + theme_bw()
...
```{r echo=FALSE}
qplot(chlorides,data = subset(df,!is.na(chlorides)), xlab="chlorides",binwidth=0.01) + theme_bw()
...
```{r echo=FALSE}
qplot(residual.sugar,data = subset(df,!is.na(residual.sugar)), xlab="residual sugar",binwidth=0.1) + theme_bw()
...
```{r echo=FALSE}
qplot(density,data = subset(df,!is.na(density)), xlab="density",binwidth=0.001) + theme_bw()
summary(df$density)
```
...
```{r echo=FALSE}
qplot(pH,data = subset(df,!is.na(pH)), xlab="pH",binwidth=0.01) + theme_bw()
summary(df$pH)
```
...
```{r echo=FALSE}
qplot(alcohol,data = subset(df,!is.na(alcohol)), xlab="Alcohol",binwidth=0.1) + theme_bw()
summary(df$alcohol)
```
...
bi_qplot <- function(x, y, z="jitter") {
  if(z=="boxplot") {
    qplot(x=x, y=y, data=df, alpha=0.01) + 
      geom_boxplot() +
      guides(alpha="none") +
      theme_bw()
  }
...
ggplot(data=df, aes(factor(quality), fill=quality.level)) + 
  geom_bar() + 
  xlab("Quality") + 
